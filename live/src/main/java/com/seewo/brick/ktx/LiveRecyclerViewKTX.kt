 ...
 *
 * @see Context.simpleRecyclerView 构造方法更简单
 */
fun <T: RecyclerItemData> Context.liveRecyclerView(
    ...
    width, height, attr, id, tag, foreground, background, padding = padding,
 ...
 *
 * @see Context.recyclerView 性能开销更少，但使用复杂不灵活，更建议使用liveSimpleRecyclerView
 */
 ...
 * @param block 针对每一个Item的数据到创建ItemView。回调输入为数据列表，列表index。不需要包裹recyclerItem。
 *
 * @see ViewGroup.recyclerView 性能开销更少，但使用复杂不灵活，更建议使用liveSimpleRecyclerView
 */
